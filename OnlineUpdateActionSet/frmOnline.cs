...
namespace OnlineUpdate
{
    public partial class frmOnline : Form
    {
        ...
        private CActionSets m_objActionSet;
            ...
            try
            {
                ...
                if (m_objShotBox == null)
                {
                    ...
                    if (!string.IsNullOrEmpty(sXml))
                    {
                        ...
                        m_objActionSet = m_objShotBox.ActionSets;
                        lstActionSets.Items?.Clear();
                        lstActionSets.DataSource=m_objActionSet;
                        lstActionSets.DisplayMember = "Name";
                        lstActionSets.ValueMember = "Index";
                    }
                }
                #endregion
            }
        ...
        private void btnUpdate_Click_1(object sender, EventArgs e)
        {
            ...
            try
            {
                if (!Equals(m_objShotBox, null))
                {
                    datavalues = new string[lstActionSets.SelectedItems.Count];
                    for (int i = 0; i < lstActionSets.SelectedItems.Count; i++)
                    {
                        datavalues[i] = (lstActionSets.SelectedItems[i] as CActionSet).Name;
                    }
                    m_objShotBox.PlayActionSet(datavalues);
                }
            }
            catch (Exception ex)
            {
